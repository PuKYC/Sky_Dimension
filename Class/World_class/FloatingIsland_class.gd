...
## 把cell从局部坐标系转换到全局坐标系
func cell_local_to_global(cell:Vector3):
		return cell*cell_size + floatingisland_AABB.position
...
func generate_cell(cell_posi: Vector3) -> bool:
		...
		if cell_posi in generation_date["generation_floatingisland_cells"]:
				var cell_posi_v3i = Vector3(cell_posi)
