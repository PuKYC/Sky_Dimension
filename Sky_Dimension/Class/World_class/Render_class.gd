...
func add_mesh(floatingisland_grid: Dictionary):
	_floatingisland_grid.merge(floatingisland_grid)
	#暂时
	generate_mash()
...
func generate_mash():
	...
	surfacetool_array[Mesh.ARRAY_TEX_UV] = PackedVector2Array()
	
	surfacetool.begin(Mesh.PRIMITIVE_TRIANGLES)
	for floatingisland in _floatingisland_grid:
		var offset = Transform3D(Basis(), floatingisland.floatingisland_AABB.position)
		
		for cell in _floatingisland_grid[floatingisland]:
			for faces in _floatingisland_grid[floatingisland][cell]:
				if faces[Mesh.ARRAY_FORMAT_VERTEX] == null and faces[Mesh.ARRAY_NORMAL] == null and faces[Mesh.ARRAY_TEX_UV] == null:
					continue
					
				surfacetool_array[Mesh.ARRAY_VERTEX].append_array(faces[Mesh.ARRAY_VERTEX]*offset)
				...
				surfacetool_array[Mesh.ARRAY_TEX_UV].append_array(faces[Mesh.ARRAY_TEX_UV])
